...
class ImageProcessingApp:
    ...
    def load_image(self):
        file_path = filedialog.askopenfilename(
            ...
            filetypes=[
                ("Tüm Görüntü Dosyaları", "*.jpg;*.jpeg;*.png;*.bmp;*.tiff"),
        ...
        try:
            # Dosyayı binary modda oku ve bir numpy arraye çevir
            with open(file_path, 'rb') as f:
                file_bytes = np.fromfile(f, dtype=np.uint8)
            
            # Numpy arrayini Opencv kullanarak bir görüntüye dönüştürme
    ...
    def _ensure_binary_image(self):
        ...
        if not self.is_binary:
            messagebox.showwarning("Uyarı", "Bu işlem yalnızca ikili (siyah-beyaz) görüntülerde çalışır.\nLütfen önce bir eşikleme metodu uygulayın.")
            return False
        return True
        
    def _get_gray_image(self):
        if len(self.processed_image.shape) == 3:
            return cv2.cvtColor(self.processed_image, cv2.COLOR_BGR2GRAY)
    ...
    def apply_filter_with_kernel(self, filter_type):
        ...
        if ksize and ksize % 2 == 1:
            if filter_type == 'average':
                self.processed_image = cv2.blur(self.processed_image, (ksize, ksize))
    ...
    def apply_shearing(self):
        ...
        if shear_factor is not None:
            ...
            new_w = int(w + h * abs(shear_factor))
            self.processed_image = cv2.warpAffine(self.processed_image, M, (new_w, h))
            self.update_processed_view()

    # --- Histogram Fonksiyonları ---
    def update_histogram(self):
        ...
        self.hist_ax.set_xlabel("Piksel Değeri")
        self.hist_ax.set_ylabel("Piksel Sayısı")
    ...
    def apply_otsu_threshold(self):
        if not self._ensure_image_loaded(): return
        gray = self._get_gray_image()
        thresh_val, self.processed_image = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        self.is_binary = True
        messagebox.showinfo("OTSU Metodu", f"Hesaplanan en iyi eşik değeri: {int(thresh_val)}")
        self.update_processed_view()
    ...
    def apply_kapur_threshold(self):
        ...
        for t in range(1, 256):
            p_obj = np.sum(hist_prob[:t])
            p_bck = np.sum(hist_prob[t:])
            
            if p_obj == 0 or p_bck == 0:
                continue
                
            h_obj_sum = 0
            for p in hist_prob[:t]:
                if p > 0:
                    h_obj_sum += (p / p_obj) * np.log(p / p_obj)
            h_obj = -h_obj_sum

            h_bck_sum = 0
            for p in hist_prob[t:]:
                if p > 0:
                     h_bck_sum += (p / p_bck) * np.log(p / p_bck)
            h_bck = -h_bck_sum
            
            total_entropy = h_obj + h_bck
            
            if total_entropy > max_entropy:
                max_entropy = total_entropy
                best_thresh = t
                
        _, self.processed_image = cv2.threshold(gray, best_thresh, 255, cv2.THRESH_BINARY)
        self.is_binary = True
        messagebox.showinfo("Kapur Entropi", f"Hesaplanan en iyi eşik değeri: {best_thresh}")
        self.update_processed_view()
    ...
    def calculate_centroid(self):
        ...
        if not contours:
            messagebox.showwarning("Bulunamadı", "Görüntüde bir nesne (kontur) bulunamadı.")
            return
        ...
        for i, c in enumerate(contours):
            ...
            if M["m00"] != 0:
                ...
                cv2.drawContours(img_with_centroid, [c], -1, (0, 255, 0), 2) # Kontur yeşil
                cv2.circle(img_with_centroid, (cX, cY), 5, (0, 0, 255), -1) # Merkez kırmızı
                
        c_largest = max(contours, key=cv2.contourArea)
        M_largest = cv2.moments(c_largest)
        if M_largest["m00"] != 0:
            cX_l = int(M_largest["m10"] / M_largest["m00"])
            cY_l = int(M_largest["m01"] / M_largest["m00"])
            messagebox.showinfo("Ağırlık Merkezi", f"Görüntüde {len(contours)} nesne bulundu.\nEn büyüğünün ağırlık merkezi: ({cX_l}, {cY_l})")
        
        self.processed_image = img_with_centroid
        self.is_binary = False # Artık renkli olduğu için
        self.update_processed_view()


    def extract_skeleton(self):
        if not self._ensure_binary_image(): return
        img_norm = self.processed_image.copy()
        img_norm[img_norm == 255] = 1
...
if __name__ == "__main__":
    ...
    try:
        import cv2, numpy, matplotlib, skimage, PIL
    except ImportError as e:
        messagebox.critical("Eksik Kütüphane", f"Lütfen gerekli kütüphaneleri kurun: {e.name}\n\nKomut: pip install opencv-python numpy matplotlib scikit-image Pillow")
        exit()
